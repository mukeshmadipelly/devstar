<script>
  ...
  async function processBoomerang(frames) {
    ...
    // Reverse frames except the last one to avoid doubling the last frame
  ...
  onMount(async () => {
    // Ensure gif.js.optimized is only imported on the client side
...
<style>
  ...
  .title {
    ...
    margin-bottom: 20px;
    font-weight: 600;
    color: #ff6347; /* Highlighted in orange */
  }
